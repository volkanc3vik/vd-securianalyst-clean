...
const TradeTracker = (() => {
  ...
  function openTrade({ sym, dir, entry, lev, margin, sl, tp1, tp2, tp3 }) {
    ...
    _showTerminal();
    _renderAll();
    // WSEngine'e otomatik subscribe et — fiyat güncellemesi için
    if (typeof WSEngine !== 'undefined') {
      try { WSEngine.subscribe(t.symFull.toLowerCase(), () => {}); } catch {}
    }
    _startUpdates();
    return t.id;
  }
...
">
      ...
      active.forEach(t => {
        try {
          if (typeof WSEngine !== 'undefined') {
            ...
            const p = d?.price || d?.lastPrice || d?.markPrice || d?.bestBid;
  ...
  function init() {
    ...
    if (active.length) {
      _showTerminal();
      _renderAll();
      // Mevcut aktif trade'ler için WSEngine'e subscribe et
      if (typeof WSEngine !== 'undefined') {
        active.forEach(t => {
          try { WSEngine.subscribe(t.symFull.toLowerCase(), () => {}); } catch {}
        });
