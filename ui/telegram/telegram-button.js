...
(function() {
  ...
  function _processAllNewCards() {
    for (const id of CONTAINER_IDS) {
      ...
      // Sadece marker'ı olmayan kartları işle (tek query)
  ...
  function _setupObserver() {
    ...
    _observer = new MutationObserver((mutations) => {
      if (!_enabled) return;
      // Eklenen node varsa schedule et — detaylı kontrol gerekmiyor,
      // _processAllNewCards zaten marker ile filtreler
      let hasAdded = false;
    ...
    _debug('observer attached on', CONTAINER_IDS);
  }
  ...
  function disable() {
    ...
    document.querySelectorAll(`[${MARKER_ATTR}="true"]`).forEach(card => {
      card.removeAttribute(MARKER_ATTR);
      const wrap = card.querySelector('.vd-tg-btn-row');
      const btn  = card.querySelector('.' + BTN_CLASS);
      if (btn) btn.remove();
      // Wrapper içinde başka eleman kalmadıysa wrapper'ı sök (opp-btn'i kart altına geri koy)
